<template>
  <div style="margin-top: 10px">
    <avue-crud :data="data" :option="option" :table-loading="loading" :page.sync="page" @on-load="onLoad"
               @sortable-change="sortableChange" @search-change="searchChange">
      ...
      <template slot-scope="scope" slot="menuLeft">
        ...
        <el-button type="success"
                   ...
                   size="small"
                   plain
                   @click.stop="allList" v-if="pid!=0">查看全部
        </el-button>
      </template>
      ...
      <template slot-scope="scope" slot="menu">
        ...
        <el-button icon="el-icon-document-copy" size="small" type="text" @click="addNext(scope)"
                   v-if="scope.row.pid===0">添加下级
        </el-button><!--目前只支持添加一级下级-->
        <el-button icon="el-icon-document-copy" size="small" type="text" @click="listNext(scope)"
                   v-if="scope.row.pid===0">查看下级
        </el-button>

        <el-button icon="el-icon-document-copy" size="small" type="text" @click="formDataBind(scope)">绑定表单
        </el-button>
        <el-button icon="el-icon-document-copy" size="small" type="text" @click="scopeDataBind(scope)">绑定作用域
        </el-button>

        <el-button icon="el-icon-document-copy" size="small" type="text" @click="handleCopy(scope)">复制
        </el-button>
    ...
    <el-dialog title="绑定表单" :visible.sync="formDatadialogTableVisible">
      <avue-crud :data="formData" :option="formDataoption" :table-loading="formDataLoading"
                 :page.sync="formDatapage" @on-load="formDataonLoad" @row-click="formDatahandleRowClick">
      </avue-crud>
    </el-dialog>

    <el-dialog title="绑定作用域" :visible.sync="scopeDatadialogTableVisible">
      <avue-crud :data="scopeData" :option="scopeDataoption" :table-loading="scopeDataLoading"
                 :page.sync="scopeDatapage" @on-load="scopeDataonLoad" @row-click="scopeDatahandleRowClick">
      </avue-crud>
    </el-dialog>
